...
export default function CodePreview({ cssCode, tailwindCode, reactCode }: Props) {
  ...
  const copy = async () => {
    await navigator.clipboard.writeText(code.replace(/^\/\*[\s\S]*?\*\/\n*/gm, "").trim());
    setCopied(true);
  ...
  return (
    <div className="bg-zinc-900 dark:bg-zinc-950 rounded-xl overflow-hidden border border-zinc-800">
      <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-800/40 dark:bg-zinc-900/60">
        ...
        <button
          onClick={copy}
          className="flex items-center gap-1.5 px-3 py-1 text-[11px] font-medium bg-indigo-600 hover:bg-indigo-500 text-white rounded-md transition-all duration-200"
        >
          {copied ? (
            <>
