...
const NEUMORPH_OPTIONS = Object.keys(NEUMORPH_PRESETS).map((k) => ({
  value: k,
  label: k,
}));
...
export default function Shadow() {
  ...
  const formatShadow = layers
    .map((l) => {
      const alpha = Math.round(l.opacity * 255)
        .toString(16)
        .padStart(2, "0");
      const prefix = l.inset ? "inset " : "";
      return `${prefix}${l.x}px ${l.y}px ${l.blur}px ${l.spread}px ${l.color}${alpha}`;
    })
    .join(",\n       ");

  const cssCode = `background: ${bgColor};
box-shadow: ${formatShadow};`;

  const twShadow = formatShadow.replace(/\n\s+/g, " ");

  const tailwindCode = `/* 任意值语法（单行） */
<div className="shadow-[${twShadow}] bg-[${bgColor}]" />
    ...
    <>
      ...
      <div className="flex flex-wrap gap-1">
        {NEUMORPH_OPTIONS.map((opt) => (
          ...
          >
            {PRESET_LABELS[opt.value] ?? opt.value}
          </button>
        ))}
      </div>
      ...
      <SectionLabel
        label="Layers"
        badge={`${layers.length}`}
        action={{ label: "+ Add", onClick: addLayer, disabled: layers.length >= MAX_LAYERS }}
      />
      ...
      <div className="space-y-3 max-h-72 overflow-y-auto custom-scrollbar">
        {layers.map((l, i) => (
          <div
            key={l.id}
            className="p-3 border border-zinc-100 dark:border-zinc-800 rounded-xl space-y-2.5 bg-zinc-50/50 dark:bg-zinc-900/50"
          >
      ...
      <div className="flex gap-3 pt-1">
        <div className="flex items-center gap-1.5">
          ...
          <input
            type="color"
            value={bgColor}
            onChange={(e) => { setBgColor(e.target.value); setActiveNeumorph(null); }}
            className="w-7 h-7"
          />
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <input
            type="color"
            value={boxColor}
            onChange={(e) => { setBoxColor(e.target.value); setActiveNeumorph(null); }}
            className="w-7 h-7"
          />
        </div>
      </div>
        ...
        <div className="flex items-center justify-center p-8 rounded-2xl transition-colors" style={{ background: bgColor }}>
          ...
          >
            <span
              ...
              style={{ color: hasInset ? "#9ca3af" : "#6b7280" }}
            >
              {statusLabel}
            </span>
          </div>
